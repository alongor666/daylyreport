...
## 技能文档最佳实践（摘要） — 2025-11-09

- 一屏摘要：每个技能以 5–8 条要点说明“目的/口径/输入输出/边界/验收”，便于快速审阅与执行。
- 反模式清理：删除教程式长文与重复解释，保留可操作的规范与约束；详细说明移至归档。
- 口径与契约：明确数据口径、字段来源与异常值处理（分母为 0、裁剪范围等），与后端契约一致。
- 验收清单：列出可验证的用例与预期结果；新增技能必须对应 1–2 条可复现验收。
- 术语统一：中文术语与内部语义分离（UI：中文；内部：`D/D-7/D-14` 等），禁止混用造成逻辑替换。
- 维护关联：新增或变更技能需同步更新 `CLAUDE.md/README.md` 的原则与维护规范，并在 `问题记录表.md` 登记影响项。
...
## 技术约束补充（日期与去重向量化） — 2025-11-09

- 日期口径（按天）：
  - 所有“当日/近7天/近30天”计算采用 `Series.dt.normalize()` 对齐到“日”后比较或构造区间掩码；禁止使用 `.dt.date` 与 `datetime.date()` 进行混合类型比较。
- 星期索引与周窗口：
  - 后端统一使用 `Series.dt.dayofweek`（周一=0）进行周内分组与对齐；`D/D-7/D-14` 标签仅为内部语义来源，不受 UI 文案影响。
- 去重规则（保持业务口径）：
  - 保单映射（保单→业务员）：保留首条（`keep='first'`）。
  - 历史合并（保单+投保确认时间）：保留最新一条（`keep='last'`）。
  - 实现方式：使用 `duplicated(...)` 生成掩码后反选，替换 `drop_duplicates(...)`，以提升稳健性与版本兼容。
- 验收补充：
  - 周对比与 KPI 返回在上述约束下值域与结构不变；抽样校验星期索引与重复样本处理符合预期。
...
### 文档展示最佳实践补充（2025-11-09）

- 对于超大枚举型取值（如精友车型 6k+ 项），在主说明文档中不展开明细；只保留统计信息与字段解释，明细通过归档或独立清单查看。
## 需求补充：业务员筛选的输入与后端兼容

- 需求：前端业务员筛选可采用“中文姓名”或“工号+姓名”两种形式；后端需兼容并返回一致结果。
- 设计要点：
  - 前端使用 policy-mapping 的姓名键进行机构/团队联动；
  - 后端在筛选时应通过映射反查或中文提取匹配，避免值格式不一致导致无数据。
- 验收标准：
  - 在仅姓名传值时，KPI与图表数据可正确返回；
  - 与工号+姓名传值效果一致（同一人员）。