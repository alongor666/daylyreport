...
- 更新节律：每次开发前后必须阅读并更新本目录下文档（问题记录表/CLAUDE/PRD-best/PROGRESS/README），保持口径与规范一致。
- 写作风格：一屏摘要（5–8 条要点），动作导向，明确“目的/口径/输入输出/边界/验收”；避免教程化长文与重复解释，详细说明转入归档。
- 术语与语义：UI 采用中文标签；内部语义统一使用稳定标识（如 `D/D-7/D-14`），禁止用中文文案替换内部计算或数据结构。
- 审查四问：是否常见？是否独特？是否可验证？投入产出比是否合理？不满足者降级或合并处理，避免文档与实现膨胀。
- 变更登记：重大变更需在 `问题记录表.md` 登记影响项与风险，并在 `PROGRESS.md` 追加进展与下一步行动；必要时附预览链接与验收清单。
- 联动维护：新增技能必须同步更新 `PRD-best.md`（口径与验收）、`CLAUDE.md`（原则与约束）、`README.md`（维护规范），并清理旧版冗长说明。
- 验收与回归：新增/变更后进行预览（无 UI 漂移、单位/格式一致），抽样核对分子/分母与比例裁剪策略（分母为 0 返回 0.0、结果裁剪至 `[0,1]`）。
