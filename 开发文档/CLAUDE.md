...
- 结果导向：明确产出与验收标准，拒绝“教程化堆砌”和笼统概念，优先保留可执行规范与约束。
- 四问法：是否常见？是否独特？是否可验证？投入产出比是否合理？不满足者应降级或合并处理。
- 写作规范：一屏摘要、动作导向、边界与异常值可快速核验；避免长篇背景描述与重复解释。
- 文档整合：`归档文件/技能批判.md` 已重写为“快速参考手册”，本文件与 `PRD-best/README` 已同步原则与维护规范。
- 维护策略：新增技能必须同时更新 PRD 验收、README 维护规范，并在问题记录表登记影响与风险项。
