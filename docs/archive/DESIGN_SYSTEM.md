...
- 维度新增：`保单号`（唯一标识），以下拉或搜索选择方式呈现；
- 联动行为：选择保单号后，自动填充并锁定 `业务员/三级机构/团队`，避免用户手动制造不一致；
- 禁用规则：当保单号非空时，禁用机构与团队选择控件；
- 校验提示：若数据中的机构/团队与映射不一致，显示计数与简要说明；
- 数据源：`GET /api/filter-options`（含保单号列表）与 `GET /api/policy-mapping`（映射详情）。
